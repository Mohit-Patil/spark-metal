...
# Mirrors inspect-grouped-aggregates.sh: stock plans (no spark.sql.extensions)
# so the probe classifies the join regions SparkMetalColumnarRule WOULD see,
# not ones it has already replaced.
spark-submit \
  --master 'local[8]' \
  --driver-memory 6g \
  ...
  --class io.github.mohitpatil.sparkmetal.BroadcastJoinShape \
